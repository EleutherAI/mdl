...
def main(args):
    ...
    # for erasure_method in ["Q-LEACE", "None", "Linear", "LEACE"]:
    for erasure_method in ["None", "LEACE"]:
        print(f"Erasure: {erasure_method}")

        if erasure_method != "None":
            fitter_class = {
                "Linear": OracleFitter,
                "Q-LEACE": QuadraticFitter,
                "LEACE": LeaceFitter,
            }[erasure_method]
            fitter = fitter_class.fit(X_train, Y_train)
            eraser = fitter.eraser
            X_train_ = (
                eraser(X_train, Y_train)
